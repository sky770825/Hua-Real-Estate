...
:start
echo ================================
echo 🏠 華地產專用網站 - GitHub 版本管理工具 (增強版)
echo ================================
echo.
...
if "%choice%"=="6" goto check_git_status
...
git --version >nul 2>&1
if errorlevel 1 (
    echo ❌ Git未安裝或未配置到PATH
    echo 請先安裝Git: https://git-scm.com/
    pause
    goto start
)
echo ✅ Git已安裝
...
if errorlevel 1 (
    ...
    echo.
    echo  建議操作：
    echo 1. 使用 "3. 建立備份" 建立版本
    echo 2. 或使用 "5. 初始化Git倉庫" 部署當前版本
    echo.
    pause
    goto start
)
